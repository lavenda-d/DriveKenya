...
const NotificationCenter = ({ isOpen, onClose, user }) => {
  ...
  const getNotificationStyles = (type, priority, isRead) => {
    ...
    if (isRead) {
      baseStyles += "bg-muted/30 border-border/50 opacity-60 ";
    } else {
      baseStyles += "bg-card border-border shadow-lg hover:shadow-xl ";
    }

    if (priority === 'high') baseStyles += "border-destructive/30 ";
  ...
  return (
    <div className="fixed inset-0 bg-background/80 backdrop-blur-md flex items-start justify-center pt-24 p-4 z-[9999] font-sans">
      <div className="bg-popover/95 backdrop-blur-2xl border border-border rounded-3xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[85vh] overflow-hidden relative">
        ...
        <div className="flex items-center justify-between p-8 border-b border-border relative z-10">
          ...
          <button
            onClick={onClose}
            className="text-muted-foreground hover:text-foreground bg-muted/50 hover:bg-muted p-2.5 rounded-full transition-all group"
          >
        ...
        {/* Filter Tabs */}
        <div className="flex items-center p-4 bg-muted/30 overflow-x-auto hide-scrollbar relative z-10 border-b border-border gap-2">
          {[
        ...
        {/* Quick Actions */}
        {notifications.filter(n => !n.is_read).length > 0 && (
          <div className="p-4 bg-primary/5 border-b border-primary/10 relative z-10 flex justify-between items-center">
            <p className="text-xs text-primary/80 font-medium italic">You have unread news...</p>
            <button
              onClick={markAllAsRead}
              className="text-xs text-primary hover:text-primary/80 font-bold flex items-center bg-primary/10 px-3 py-1.5 rounded-lg border border-primary/20 transition-all active:scale-95"
            >
        ...
        {/* Notifications List */}
        <div className="flex-1 overflow-y-auto p-6 space-y-4 relative z-10 bg-muted/10">
          {loading ? (
            <div className="flex flex-col items-center justify-center p-12 space-y-4">
              <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-primary"></div>
              <span className="text-muted-foreground text-sm font-medium">Syncing updates...</span>
            </div>
          ) : notifications.length === 0 ? (
            <div className="flex flex-col items-center justify-center p-16 text-center">
              <div className="bg-muted/30 p-6 rounded-full mb-6 border border-border">
                <Bell size={48} className="text-muted-foreground/40" />
              </div>
              <h3 className="text-xl font-bold text-foreground mb-2">Zero distractions</h3>
              <p className="text-muted-foreground text-sm max-w-[250px]">You've handled all your notifications. Great job!</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-start justify-between relative z-10">
                    <div className="flex items-start space-x-4 flex-1">
                      <div className="p-2.5 bg-background rounded-xl border border-border shrink-0">
                        {getNotificationIcon(notification.type)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <h4 className={`text-sm font-bold truncate ${notification.is_read ? 'text-muted-foreground' : 'text-foreground'}`}>
                            {notification.title}
                          </h4>
                          <span className="text-[10px] font-bold text-muted-foreground/60 shrink-0 ml-2">
                            {formatTimeAgo(notification.created_at)}
                          </span>
                        </div>
                        <p className={`text-sm leading-relaxed ${notification.is_read ? 'text-muted-foreground/60' : 'text-muted-foreground'}`}>
                  ...
                  {/* Actions Area */}
                  {!notification.is_read && (
                    <div className="mt-4 pt-4 border-t border-border/50 flex items-center space-x-3">
        ...
        {/* Footer */}
        <div className="p-6 border-t border-border bg-muted/30 relative z-10">
          <p className="text-[10px] text-muted-foreground/60 text-center font-bold uppercase tracking-[0.2em]">
